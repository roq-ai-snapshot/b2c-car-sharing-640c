...
import { getAccessRights } from 'apiSdk/access-rights';
import { AccessRightInterface } from 'interfaces/access-right';

import { InvitationListPage as ShowCaseList } from 'pages/invitations';
...
function DashboardPage() {
  ...
  const accessRightStat = useSWR<PaginatedInterface<AccessRightInterface>>(
    () => `/access-rights`,
    () => getAccessRights({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Access Rights',
      result: accessRightStat?.data?.totalCount,
      isLoading: accessRightStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`invitations`} passHref legacyBehavior>
                  <Button
